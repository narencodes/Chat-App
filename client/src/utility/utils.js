...
let checkIfObjectId = id => {
	return id.match(/^[0-9a-fA-F]{24}$/);
}

let getFormattedTime = (time = Date.now(), isRailwayTime = false) => {
	...
	let formattedHours = !isRailwayTime && hours > 12 ? hours - 12 : (hours === 00) ? 12 : hours;
	return `${formattedHours}:${minutes} ${isRailwayTime ? '' : ( hours >= 12 ? 'pm' : 'am' ) }`;
}

let getDateUtilities = time => {
	time = new Date(parseInt(time));
	let [day, month, date, year] = time.toDateString().split(" "); // Wed Jun 10 2020 
	let [ dateNumber, monthNumber ] = time.toLocaleDateString().split("/"); // 10/06/2020;
	return {
		day,
		month,
		date,
...
let getFormattedLastSeen = (time = Date.now()) => {
	let text = "last seen";
